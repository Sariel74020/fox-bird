...
// 专辑名数组
let albums=['酒狂','半山听雨','多远都要在一起','A Thousand Years','永不失联的爱','水星记','我用什么把你留住'];
// 歌曲信息数组
let track_names=['龚一 - 酒狂','杨青 - 半山听雨','G.E.M.邓紫棋 - 多远都要在一起','Christina Perri - A Thousand Years','周兴哲 - 永不失联的爱','郭顶 - 水星记' ,'福禄寿 - 我用什么把你留住'];
let Songname=['jiukuang','banshantingyu','duoyuandouyaozaiyiqi','A Thousand Years','yongbushiliandeai','shuixingji','woyongshenmobaniliuzhu']
...
function selectTrack(flag){
    ...
    if(cur_index>-1 && cur_index<albums.length){
        ...
        let cur_song_name = Songname[cur_index];
        // 当前歌曲信息(歌手 - 歌名)
        let cur_track_name=track_names[cur_index];
        // 设置音频路径
        audio.src='../music/'+cur_song_name+'.mp3';
        if(flag!=0){
            // 当切换上一首,下一首时,自动播放
            audio.play();
            player_track.classList.add('active');
